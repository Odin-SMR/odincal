...
class Level0File:
    ...
    def getIndex(self):
        ...
        if len(data) == self.blocksize:
            ...
            self.sync = words[0]
            self.stw  = words[2]*65536L+words[1]
            self.user = words[3]
...
class SHKfile(Level0File):
    ...
    def getLOfreqs(self):
        sub  = lambda w: w & 0x000f
        freq = lambda hro, pro, m: ((4000.0+hro)*m + pro/32.0+100.0)*6.0e6
        self.rewind()
        stw = []
        aside = []
        bside = []
...
class ACfile(Level0File):
    ...
    def __init__(self,file):
        ...
        if self.type != 'AC1' and self.type != 'AC2':
            raise TypeError
        if   self.type == 'AC1': self.block0 = 0x7380
        elif self.type == 'AC2': self.block0 = 0x73b0
    ...
    def IntTime(self,words):
        ...
        if prescaler >= 2 or prescaler <= 6:
            ...
            # print "(%08X) prescaler (%d) out of range" % (self.stw, prescaler)
            samples = 0L
...
class AOSfile(Level0File):
    ...
    def Type(self, words):
        ...
        if type == 'REF':
            ...
            if   calmirror == 1: type = 'SK1'
            elif calmirror == 2: type = 'CAL'
            elif calmirror == 3: type = 'SK2'
        if words[30+2] == 0: type = 'DRK'
        if words[30+4] == 1: type = 'CMB'
...
def getSideBand(rx, LO, ssb):
    ...
    for i in range(-2,3):
        ...
        if abs(abs(s3900)-3.9) < abs(abs(d)-3.9):
            d = s3900;
    if d < 0.0: IFfreq = -3900.0e6
    else:       IFfreq =  3900.0e6
