...
if backend == 'AC1':
    file = "/OdinData/AERO/7.0/AC1/"+hex_orbit[0:2]+"/OB1B"+hex_orbit+".LOG"
    cal = open(calfileACn,"r")
elif backend == 'AC2':
    file = "/OdinData/AERO/7.0/AC2/"+hex_orbit[0:2]+"/OC1B"+hex_orbit+".LOG"
    cal = open(calfileACn,"r")
else:
    file = "/OdinData/AERO/7.0/FBA/"+hex_orbit[0:2]+"/OD1B"+hex_orbit+".LOG"
...
if logexist == 1:
    buf = os.stat(file)
    created = time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(buf[stat.ST_CTIME]))
...
# ------------------------------------------------------------------------------
    ...
    line_spectra = "0"
    if backend == 'AC1':
        file_old = "/OdinData/AERO/AC1/"+hex_orbit[0:2]+"/OB1B"+hex_orbit+".TXT"
        file_new = "/OdinData/AERO/7.0/AC1/"+hex_orbit[0:2]+"/OB1B"+hex_orbit+".TXT"
    elif backend == 'AC2':
        file_old = "/OdinData/AERO/AC2/"+hex_orbit[0:2]+"/OC1B"+hex_orbit+".TXT"
        file_new = "/OdinData/AERO/7.0/AC2/"+hex_orbit[0:2]+"/OC1B"+hex_orbit+".TXT"
    else:
        file_old = "/OdinData/AERO/FBA/"+hex_orbit[0:2]+"/OD1B"+hex_orbit+".TXT"
        file_new = "/OdinData/AERO/7.0/FBA/"+hex_orbit[0:2]+"/OD1B"+hex_orbit+".TXT"
...
query_select = "SELECT id FROM "+table2+" WHERE orbit = '"+sys.argv[1]+"' AND backend = '"+sys.argv[2]
...
if not rows:
    ...
    query = query + " (orbit,backend,freqmode,nscans,nspectra,calversion,attversion,processed,start_utc,stop_utc) VALUES "
    query = query + " ('%d','%s','%s','%d','%d','%d','%s','%s','%s','%s'); " % (orbit,backend,freqmode,line_scan,spectra,calversion,attversion,created,start_utc,stop_utc)  
else:
    id = rows[0]
...
try:
    conn = MySQLdb.connect (host=remote_host, user=remote_user, passwd=remote_pw, db=remote_db)
...
if not rows:
    ...
    query = "INSERT INTO "+table2 
    query = query + " (orbit,backend,freqmode,nscans,nspectra,calversion,attversion,processed,start_utc,stop_utc) VALUES "
    query = query + " ('%d','%s','%s','%d','%d','%d','%s','%s','%s','%s'); " % (orbit,backend,freqmode,line_scan,spectra,cal,attversion,created,start_utc,stop_utc)  
    buf = "inserting"
else:
    id = rows[0]
