...
class OrbitProcessor:
    ...
    def tuning(self, s):
        ...
        if not hasattr(self, 'rx'):
            self.rx = {}
        if not self.rx.has_key(s.frontend):
            shk = self.shk
...
#                        if s.intmode & AC_UPPER: IFfreq = IFfreq*3.6/3.9
                        if s.intmode & ADC_UPPER: IFfreq = IFfreq*3.6/3.9
                        else:                     IFfreq = IFfreq*4.2/3.9
                    elif s.backend == 'AC2':
#                        if s.intmode & AC_UPPER: IFfreq = IFfreq*4.2/3.9
                        if s.intmode & ADC_UPPER: IFfreq = IFfreq*4.2/3.9
    ...
    def cleancal(self, cal, ref):
        ...
        if n1 == 0:
            ...
            for i in range(nc-1,-1,-1):
                if mc[i] < Tmin or mc[i] > Tmax or abs((mc[i]-tsys)/tsys) > 0.02:
...
#        print "medTsys", C
    ...
    def FCalibrate(self, s, LOfreq, IFfreq):
        ...
        if s.frontend in rxs.keys() and self.shk:
            if not hasattr(self, 'temp'):
                self.temp = {}
            if not self.temp.has_key('PllBox'):
    ...
    def doAC(self, file):
        ...
        ac = odin.ACfile(file)
        stw0 = self.stw0 % 0x100000000L
        stw1 = self.stw1 % 0x100000000L
        while 1:
            try:
                s = ac.NextSpectrum()
                print s
    ...
    def getLoadTemp(self):
        ...
        if not hasattr(self, 'temp'):
            self.temp = {}
        if not self.temp.has_key('hotLoad'):
    ...
    def process(self, mode = None):
        ...
        for s in all:
            ...
            if rx in ['119','495','549','555','572','   ']:
                if not self.spectra.has_key(rx):
                    self.spectra[rx] = []
    ...
    def attitude(self):
        ...
        if attfiles:
            ...
            row = 0
            for key in keys:
                (year,mon,day,hour,min,secs,stw,orbit,qt,qa,qe,gps,acs) = tbl[key]
            ...
            odin.Info("orbit %5d lasted for %5.2f minutes" % (self.orbit,mins))
                    ...
                    for s in self.spectra[rx]:
                        ...
                        if i > 0 and i < row:
                            ...
                            if s.discipline == 'ASTR':
                                s.restfreq = s.skyfreq/(1.0-s.vlsr/2.99792456e8)
                            else:
...
if __name__ == "__main__":
    if len(sys.argv) > 3:
        ...
        if not discipline in ['ASTR','AERO']:
            usage()
        # pdb.set_trace()
