...
#  ---------------------------------------------------------
if len(sys.argv) != 4:
    print "Use: $ python " +sys.argv[0]+ " <AC1|AC2|FBA> <decimal orbit> <verify|update|local|remote>"
    sys.exit()
backend = sys.argv[1]
orbit = sys.argv[2]
mode = sys.argv[3]
if ( ((backend != 'AC1') & (backend != 'AC2') & (backend != 'FBA')) |
     ((mode != 'verify') & (mode != 'update') & (mode != 'local') & (mode != 'remote')) ):
...
if ((mode == 'update') | (mode == 'local')):
    ...
    query = "SELECT id FROM "+table1+" WHERE orbit='"+orbit+"' AND backend='"+backend+"'"
    q = db.query(query)
    rows = q.getresult()
    id = rows[0]
    cid = str(id[0])
...
if ((mode == 'update') | (mode == 'remote')):
    ...
    #  ---------------------------------------------------------
    query  = "UPDATE "+table2+" SET errmsg = '"+msgstring+ "' WHERE id = "+str(id)
    #print "remote query: ",query
    cursor.execute(query)
    cursor.close()
