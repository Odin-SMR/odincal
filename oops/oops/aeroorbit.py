...
#                       teff, s.inttime, teff/s.inttime)
        ...
        for s in calibrated:
            ...
            if s.type == 'SPE':
                s.data = choose(equal(s.data,0.0), ((s.data-Tspill)/eta, 0.0))
    ...
    def getScans(self, sig):
        ...
        h0 = min(h)
        h1 = max(h)
        odin.Info("height range: %10.4f %10.4f %10.4f %10.4f" % \
    ...
    def PDCname(self):
        if   self.backend == 'AOS': bcode = 'A'
        elif self.backend == 'AC1': bcode = 'B'
        elif self.backend == 'AC2': bcode = 'C'
            ...
            if total > 0:
                ...
                if len(scans) == 1:
                    ...
                    for i in range(1,len(scans)):
                        ...
                        ao.LogScan(i,first,last,fm,total,specs[first],specs[last])
                    first = scans[i][0]
                    last  = total-1
                    fm = scans[i][1]
                    if last-first > 10:
                        # print "log scan", first+2, last+1, total
                        ao.LogScan(len(scans),first,last,fm,total,specs[first],specs[last])
