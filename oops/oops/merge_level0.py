...
for row in q.getresult():
        ...
        if ( (old_setup != setup) or ((setup != "W3A,W4A") and (setup != "W5A,W5B")) ):
                ...
                query = "INSERT INTO " + table_local_ut + " (start_orbit,stop_orbit,start_utc,stop_utc,vdop,mode,setup) "
                query = query + " VALUES (%10.2f,%10.2f,'%s','%s',%.1f,'%s','%s'); " % (old_start_orbit,old_stop_orbit,old_start_utc,old_stop_utc,float(old_vdop),old_mode,old_setup)
                print query
                q = db.query(query)
                                                
                #  insert row into remote table
                query = "INSERT INTO " + table_remote_ut + " (start_orbit,stop_orbit,start_utc,stop_utc,vdop,mode,setup) "
                query = query + " VALUES (%10.2f,%10.2f,'%s','%s',%.1f,'%s','%s'); " % (old_start_orbit,old_stop_orbit,old_start_utc,old_stop_utc,float(old_vdop),old_mode,old_setup)
                print query
                cursor.execute (query)
