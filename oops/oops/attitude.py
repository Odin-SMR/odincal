...
class AttitudeParser:
    ...
    def __init__(self, files, stw0=0L, stw1=0x800000000L):
        ...
        for file in files:
            ...
            t = self.getLine()
            while t:
                (year,mon,day,hour,min,secs,stw,orbit,qt,qa,qe,gps,acs) = t
    ...
    def getLine(self):
        ...
        qa = (float(cols[10]),float(cols[11]),float(cols[12]),float(cols[13]))
        ...
        if len(cols) == 37 and int(cols[34]) == 5:
            ...
            acs = 0.0
        self.t1 = (year,mon,day,hour,min,sec,stw,orbit,qt,qa,qe,gps,acs)
        return self.t1
...
if __name__ == "__main__":
    ...
    for file in sys.argv[1:]:
        ...
        (year,mon,day,hour,min,secs,stw,orbit,qt,qa,qe,gps,acs) = t[keys[0]]
        msg = " %08X %10d %10.3f %4d-%02d-%02d %02d:%02d:%06.3f" % \
              (stw, stw, orbit, year, mon, day, hour, min, secs)
        print msg

        (year,mon,day,hour,min,secs,stw,orbit,qt,qa,qe,gps,acs) = t[keys[-1]]
        msg = " %08X %10d %10.3f %4d-%02d-%02d %02d:%02d:%06.3f" % \
              (stw, stw, orbit, year, mon, day, hour, min, secs)
        print msg
