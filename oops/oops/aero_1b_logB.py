...
try:
    ...
    print sys.argv[0] +" Oops! Open error on "+ file_name +" ===> No logging."
    sys.exit()
...
uploaded = time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(file_stats[stat.ST_CTIME]))
...
print sys.argv[0]+" updating local table "+table+" for orbit "+sys.argv[1]+" and backend "+sys.argv[2]
print query
q = db.query(query)
db.close()
...
if (n == 0):
    ...
    query = query + " logname=('%s') " % logname
query = query + " WHERE id=('%s') " % id
print sys.argv[0]+" updating remote table "+table+" for orbit "+sys.argv[1]+" and backend "+sys.argv[2]
print query
