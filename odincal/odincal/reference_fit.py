...
class Ref_fit():
    ...
    def __init__(self):
        self.lamb=100*16.0 #a parameter that control the weighting exp(-t0/lamb)
    def interp(self,t,y,t1,tint,start):
        ...
        if 0:
            y1=y1.swapaxes(0,1)
            plt.subplot(3,1,1)
            plt.plot(t,numpy.mean(y[:,0:112],1))
            plt.plot(t[index],numpy.mean(y1[:,0:112],1),'*') 
            plt.plot(t1,numpy.mean(self.c0[0:112],0),'ks')
            plt.ylabel('mean of band 1')
            mindiff=numpy.min(numpy.abs(t1-t))
            plt.subplot(3,1,2)
            plt.plot(t[index],numpy.mean(y1[:,0:112],1),'*') 
            plt.plot(t1,numpy.mean(self.c0[0:112],0),'ks')
            plt.ylabel('mean of band 1')
            plt.subplot(3,1,3)
            plt.plot(t[index],self.w,'*')
            plt.xlabel('stw')
            plt.ylabel('w')
            #plt.subplot(4,1,4)
            #plt.plot(range(896),self.c0,'g-')
            #s=y[numpy.abs(t1-t)==numpy.min(numpy.abs(t1-t)),:][0]
            #plt.plot(range(896),s,'r-')
            plt.show()
