        ...
        for item in data.keys():
            if item=='spectra':
                spec=N.ndarray(shape=(row['channels'],),dtype='float64',
                           buffer=con.unescape_bytea(row['spectra']))
                data[item].append(spec)
            elif item=='ssb_fq':
                ssb_fq=N.array(
                    eval(row['ssb_fq'].replace('{','[').replace('}',']')))*1e6
...
def main():
    ...
    for row in result:
        ...
        for tind in range(len(tcalvec)-1):
            ...
            'median_spectra' :con.escape_bytea(medianTb.tostring()),
            'mean_spectra' :con.escape_bytea(meanTb.tostring()),
